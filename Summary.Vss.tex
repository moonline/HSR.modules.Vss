...
\ch{Koordination zwischen Prozessen}
	\stdImg{v10.1}{Beispiel verteilte Synchronisation}
	\stdImg{v10.2}{Anforderungen Mutual Exclusion ME}

	\se{Verteilter wechselseitiger Ausschluss}
		\sse{Zentraler Server}
			\stdImg{v10.3}{ gegenseitiger Ausschluss-Token für ein Set von Prozessen}
			\sss{Probleme}
				\uli{
					\li P3 verlässt critical Section nicht \ra\ System bleibt stehen
				 	\li Mehrere Tokens im System \ra\ Konflikt
				 	\li Token geht verloren \ra\ System blockiert
				}
			 
		\sse{ Token für den wechselseitigen Ausschluss}
			\stdImg{v10.4}{}
			\sss{Probleme}
				\uli{
					\li Jeder daran beteiligt, auch wenn er nicht in die critical Section will
					\li Jede muss Token empfangen/senden
					\li P3 fällt aus: P2 kann P4 nicht erreichen
					\li Token geht verloren \ra\ jemand muss neuen Token generieren
					\li Mehrere Tokens sind im Umlauf
				}

		\sse{Ricart und Agrawaka's Algorithmus}
			\uli{
				\li Wer in die critical Section will, sendet einen Multicast an alle
				\li Sobald alle geantwortet haben, tritt er ein
				\li Prozess, der schon in CS ist, antwortet erst, wenn er wieder draussen ist
			} 
 			\stdImg{v10.5}{Multicasting und logische Uhren}
 			\uli{
 				\li Schwierigkeit: Anzahl Teilnehmer zu bestimmen. Multicast weis normalerweise nicht, wie viele in Multicastgruppe sind
 			}
			\stdImg{v10.6}{Multicast synchronisation Beispiel}
			\stdImg{v10.7}{Verteilte Lösung}
			\uli{
				\li Anzahl Prozesse müssen bekannt sein ohne zentralen Koordinator
				\li Single Point of failure wurde ersetzt mit n points of failure
				\li Möglichkeit: Mehrheitsvoting statt alle Antworten
			}
 
		\sse{Maekawa's Algorithmus - Mehrheitsvoting}
			\uli{
				\li Nur die Prozesse werden beteiligt, die auch die Ressource wollen
				\li Probleme: Prozess stürzt ab
			}
			\stdImg{v10.8}{Maekawa's Algorithmus}

 		\sse{Koordinator wählen}
 			\uli{
 				\li Jeder beteiligte kann eine Wahl auslösen
 				\li Dazu muss jeder Prozess eindeutig identifizierbar sein
 				\li Falls der Koordinator ausfällt, muss ein neuer gewählt werden
 			}
 			\sss{Voraussetzungen}
 				\uli{
 					\li Jeder Prozess besitzt ein Label mit dem Wert des nicht abgestürzten Prozesses mit der grössten ID
 					\li Alle Prozesse pi nehmen an der Wahl teil und setzen irgendwann elected oder stürzen ab.

 				}
 			\sss{Wahl}
 				\stdImg{v10.9}{Wahlkreis}
 				\uli{
 					\li Wahlauslöser schreibt sich auf List und sendet Wahlanforderung an nächsten
 					\li nächster trägt sich ein
 					\li Sobald ein Knoten die Liste erhält und er schon eingetragen ist, bestimmter er das Maximum und benachrichtigt den nächsten, der wieder den nächsten benachrichtigt
 					\li 2(n-1) Nachrichten
 				}
 				\stdImg{v10.10}{Wahlalgorithmus}
 				
 		\sse{Bully Algorithmus}
 			\uli{
 				\li Jeder kennt die Nachbarn, die eine höhere ID haben
 				\li Fällt einer aus, so kann der neue Koordinator direkt bestimmt werden
 				\li n-2 Nachrichten
 				\li Jeder Knoten nimmt einen andern Knoten aus seiner liste, wenn er ihn nicht mehr erreicht
 			}	
 			\stdImg{v10.11}{Bully Algorithmus}
 			\stdImg{v10.12}{Bully Alg. Beispiel}
 
 	\se{Multicast Kommunikation}
		\sse{ISIS Algorithmus}
			\stdImg{v10.13}{ISIS Algorithmus}
			\uli{
				\li Anfrage \ra\ Vorschlag \ra\ Konsens
			}
	
	\se{Konsensbildung}
		\uli{
			\li fixe Anzahl Prozesse
			\li Nur Nachrichtenaustausche
			\li Zufällige Prozessausfälle möglich
		}
		\stdImg{v10.14}{Ergebnis: p 1 und p2 einigen sich nach dem Absturz von p3 auf ,,proceed''}
		\uli{
			\li Systeme mit drei Knoten können keinen Konsens finden
			\li Bei vier Knoten ist Mehrheitsvoting möglich
		}
		
	
\ch{Modellierung paralleler und verteilter Systeme}
	\uli{
		\li Ist es Live?
		\li Gibt es Starvation?
		\li Gibt es Deadlocks?
	}
	\expl{Nebenläufigkeit / Parallelität}{\textbf{Nebenläufigkeit (Concurrency)}:  Zwei Ereignisse sind nebenläufig, wenn sie parallel ausgeführt werden können (jedoch nicht müssen), das heisst, wenn zwischen ihnen keine kausale Abhängigkeit besteht.\\
	\textbf{Parallelität}: Zwei Ereignisse finden parallel statt, wenn sie gleichzeitig ausgeführt werden.
	}		
	\se{Petri Netze}
		\definition{Petri Netz}{Kommunizierende Automaten}
		\stdImg{v11.1}{Beispiel Petri Notation}
		\sse{Komponenten und Regeln}
			\stdImg{v11.2}{Komponenten}
			\stdImg{v11.3}{Regeln}
			\stdImg{v11.4}{Regeln}
			\stdImg{v11.6}{Vor- und Nachbereiche}
			\stdImg{v11.8}{Beispiel}
			\uli{
				\li Bei einer Transaktion können sowohl Tokens vervielfacht wie gemergt werden
			}
			
		\sse{Formalisierung}
			\stdImg{v11.5}{Formalisierung}
			\stdImg{v11.7}{Formalisierung}
		
		\sse{Erreichbarkeitsgraph}
			\exam{\stdImg{v11.9}{}}
			\stdImg{v11.10}{Unbeschränktes System}
			\stdImg{v11.11}{Weiteres Beispiel}
			\stdImg{v11.12}{Erreichbarkeitstabelle}
			\stdImg{v11.13}{Endliches System}
		
		\sse{Beschränktheit}
			\uli{
				\li  Eine Stelle heisst beschränkt, wenn die Anzahl Tokens auf ihr nie eine
gewisse Anzahl übersteigt.
				\li  Ein System heisst beschränkt wenn alle Stellen des Systems
beschränkt sind.
				\li Wenn der Graph endlich ist, ist das System beschränkt
				\li Wenn der Graph unendlich ist, ist das System unbeschränkt
			}
			\stdImg{v11.14}{}
			
		\sse{Deadlock}
			\uli{
				\li  Unter Konflikt versteht man, dass das Schalten der einen Transition, die Konzession der anderen konzessierten Transition weg nimmt. 
				\li Die Unabhängigkeit liegt offensichtlich darin, dass die betreffenden Transitionen weder um Tokens in ihrem Vorbereich, noch um freie Kapazitäten in ihrem Nachbereich  konkurrieren müssen: concurrency.
			}
			\img{img/v11.15.jpg}{}{1}{}
			\img{img/v11.16.jpg}{}{1}{}
		
		\sse{Crosstalk}
			\uli{
				\li Zwei Agenten kommunizieren über einen (zuverlässigen) Kanal.
				\li Der Kanal ist zuverlässig sofern nur eine Nachricht im Kanal ist.
				\li Crosstalk entsteht, wenn beide Agenten Nachrichten in den Kanal stecken und diese im Kanal aufeinander treffen.
				\li Lösung: Acks wie beim Wirless LAN
			}
			\stdImg{v11.17}{Crosstalk Algorithmus, der Deadlockfrei, Starvationfrei und ´´living'' ist}
		
	\se{Repetition}		
		\exam{Wieso braucht man Election in einem Verteilten System? Ein Cluster oder eine Cloud braucht immer einen Chef / Verwalter. }
		\expl{Petri Netze}{Modellierung von Systemen\\ \ra\ finden von Deadlocks, livelocks, Starvation, Fairness}
		\definition{Livelock}{Einer wartet z.B. auf Nachricht von (noch lebenden) Knoten. Weil dieser andere Knoten zu beschäftigt ist um die Nachricht zu senden, wartet/blockiert das ganze System.}
	
		
		
\ch{CSP}
	\definition{CSP}{Communicating Sequential Processes, einfache Prozessalgebra}
	\expl{LTS}{Labelled Transition System, keine Endzustände, Anzahl Zustände und Übergänge unendlich, Aktionen können synchronisiert werden. }

	%\stdImg{v12.1}{Petri Notation}
	
	\stdImg{v12.2}{CPS Notation}
	\stdImg{v12.3}{Präfix Notation}
	\stdImg{v12.4}{Labyrinth Beispiel}
	\stdImg{v12.5}{CSP Sprachelemente}
	\stdImg{v12.6}{Uhr Beispiel}
	
	\uli{
		\li Verknüpfung von zwei Prozessen P,Q ist wiederum ein Prozess
		\li CSP bewegt sich auf sehr abstraktem level
		\li STOP: Kein reguläres Prozessende (System wird abgewürgt)
		\li Rekursion: Automat in Kette einbinden:\\
		$CH5: (in5 \rightarrow out2 \rightarrow out1 \rightarrow out2 \rightarrow CHF)$
			\stdImg{v12.7}{CSP Rekursion}
		\li Choice ist ein Prozess, der zwei unterschieldiche Sachen tun kann, darum Klammer ganz aussen rum:\\
			\stdImg{v12.8}{Choice (Alternative Ausführung)}
			\stdImg{v12.9}{Choice Beispiel}
			\stdImg{v12.10}{Allgemeine Auswahl (Case)}
		$(a\rightarrow b|y\rightarrow x)$
		\li Direkte Übergänge P\ra Q sind wie bei den Petrinetzen nicht erlaubt. Es muss immer ein Event dazwischen sein.
	}
	
	\stdImg{v12.11}{CSP Axiome}
	\examp{Choice}{ Beschreiben sie einen Automaten, welcher Schokolade oder Kaffee
		liefern kann.\\
		$VM = uX: {coin, choc, coffee} (coin \rightarrow choc \rightarrow X coffee \rightarrow X )$
	}

	\uli{
		\li Traces: Spuren von Prozessen
			\stdImg{v12.12}{Traces}
	}

	\examp{Traces}{(a -> b -> STOP): {<>, < a >, < a, b >}\\
 		(a -> STOP b -> STOP): {<>, < a >, < b >}
	}
	%\stdImg{v12.13}{Trace Regeln}
	
	\stdImg{v12.14}{Kopf und Ende von Traces – Länge eines Traces}
	\stdImg{v12.15}{Kommunikations Events}
	\stdImg{v12.16}{Ein-/Ausgabe}
	\stdImg{v12.17}{Akkumulator Beispiel}
	\examp{I/O}{ Kanal: a:{0,1}, besteht aus den Events: {a.0, a.1}\\
		 Beschreiben sie einen Prozess, welches die Eingabe dupliziert: \\
		 	Dup = in ?x -> out ! x -> out ! x -> Dup\\
		 Beschreiben Sie einen Prozess, der zwei natürliche Zahlen multipliziert: \\
		 	Prod = in ? x:N -> in ? y:N -> out ! (x*y) -> STOP
	}
	
	\stdImg{v12.18}{Interne Auswahl}


	\stdImg{v13.1}{Concurrency}
	\stdImg{v13.2}{Concurrency}
	\stdImg{v13.3}{Channels}



\ch{Webservices}
	\definition{Web Service}{Ein WS Interface beschreibt eine Sammlung von Diensten,
welche den Clients zur Verfügung stehen:
		\uli{
			\li Hauptvorteil: Kann in jeder beliebigen Sprache realisiert werden
			\li Programme / Dienste
			\li Objekt-Methoden (ein Dienst umfasst in der Regel mehrere Methoden!)
			\li Datenbankzugriffe
			\li Berechnungsdienste
		}
	}
	
	\expl{WebService}{Gemeinsam ist allen Web Services, dass sie in XML spezifiziert sind
und die Nutzung/Aufrufe ebenfalls in XML angegeben werden können}

	\expl{Warum ist ein Webserver kein Remote Method Call?}{Es werden nur Servicedaten übermittelt, keine konkrete Methoden. Ein Service ist mächtiger, als ein einzelner Methodenaufruf (fasst mehrere Methoden zusammen). Zudem wird möglichst Zustandslos gearbeitet.}
		
	\uli{
		\li Http basiert, keine Probleme mit Tunneln
		\li Kein Marshalling
		\li SOAP: Zugriff auf entfernte Objekte
		\li WSDL: Webservice Description Language
		\li UDDI: Registry, mit Sammlungs von Diensten
		\li Unteren Schichten bleiben verborgen
		\li  Web Services können top down oder bottom up entwickelt werden
			\uli{
				\li Top down: ausgehend von einer (XML) Web Service Beschreibung (in
WSDL)
				\li Bottom up: ausgehend von bestehendem App in C\#, Java, Perl, ... .
			}
	...
	\sse{XML RPC}
		\uli{
			\li sehr simpel
			\li Http/XML basiert
			\li nur wenige Befehle
			\li Gesendet werden Methoden mit Parameter
			\li Empfangen werden Responses mit Values
		}

	\sse{UDDI}
		\stdImg{v14.2}{}
		\stdImg{v14.7}{Grobstruktur}

	\se{SOAP}
		\uli{
			\li (simple) Object Access Protocol
			\li Header: Login, Verschlüsselung, ...
			\li Body: Methodennamen, Parameter
		}
		\stdImg{v14.3}{}
		\stdImg{v14.4}{Anfrage an Service m}
		\stdImg{v14.5}{Response}
		
		
		\sse{WSDL}
			\uli{
				\li Sehr komplex
				\li Nicht nur Daten und Methoden, sondern auch Typen und Transport Binding können definiert werden
			}
			\stdImg{v14.6}{}
	...
	\se{REST}
		...
		\uli{
			\li über die Adressierung werden Dienste angesteuert
			\li Zustandslose Client/Server Protocol
			\li RESTful nutzt Http für:
				\uli{
					\li Daten zu speichern (POST: create / update)
					\li Daten zu lesen (GET: Query, select)
					\li Daten zu löschen (DELETE)
				}
		}
		\stdImg{v14.8}{}
